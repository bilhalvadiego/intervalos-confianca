...
IntervaloConfianca <- R6::R6Class(
    "IntervaloConfianca",
    public = list(
        gerar_ic = function(n_amostras, n, media, variancia, nivel_confianca, seed = 123) {
            # ---- Alteração Inicia ----
            # Se uma seed for fornecida, define-a para garantir a reprodutibilidade
            if (!is.null(seed)) {
                set.seed(seed)
            }
            # ---- Alteração Termina ----
